...
import { ResponseStarWars } from './interfaces/star-wars.interfaces';
...
(() =>{
    ...
    getRequest(`${SW_API}/people/2`).pipe(
       ...
       switchMap((val)=>getRequest(val.species).pipe(
        map(val2 =>({
            personaje:val,
            especie: val2
        }))   
    ...
    ).subscribe( ({personaje,especie})=>console.log(personaje,especie) )           // ==
    // =======================================
